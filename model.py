...
class DocREModel(nn.Module):
    def __init__(self, config, model, emb_size=768, ffn_dim=1024, block_size=64, num_labels=-1):
        super().__init__()
        self.config = config
        self.model = model
        ...
        self.extractor_1 = nn.Linear(3 * config.hidden_size, emb_size)
        # self.entity_pair_interactor = TransformerEncoderLayer(d_model=config.hidden_size, nhead=16, batch_first=True)
        # self.htr_interactor = MultiHeadAttention(config,
        #                                          emb_size,
        #                                          emb_size,
        #                                          emb_size,
        #                                          config.num_attention_heads)
        # self.rht_interactor = MultiHeadAttention(config,
        #                                          emb_size,
        #                                          emb_size,
        #                                          emb_size,
        #                                          config.num_attention_heads)
        self.extractor_2 = nn.Linear(config.hidden_size, emb_size)
        self.interactor = nn.TransformerDecoderLayer(d_model=emb_size,
                                                     nhead=12,
                                                     dim_feedforward=ffn_dim,
                                                     activation="gelu",
                                                     batch_first=True)
        #self.htr_interactor_norm = LayerNorm(emb_size, eps=1e-5)
        #self.rht_interactor_norm = LayerNorm(emb_size, eps=1e-5)
    ...
    def forward(self,
        ...
        for i in range(input_ids.size(0)):
            ...
            htr = torch.tanh(self.extractor_1(torch.cat([hss[i], tss[i], rss[i]], dim=1)))
            hts.append(htr)
        ...
        rels = torch.tanh(self.extractor_2(rels))
        ...
        hts = self.interactor(tgt=hts, memory=rels, tgt_key_padding_mask=hts_attn_mask!=1, memory_key_padding_mask=rel_attention_mask!=1)
        rels = self.interactor(tgt=rels, memory=hts, tgt_key_padding_mask=rel_attention_mask!=1, memory_key_padding_mask=hts_attn_mask!=1)


        #hts_1 = self.htr_interactor(hts, rels, hts_attn_mask, rel_attention_mask)[0]
        #rels_1 = self.rht_interactor(rels, hts, rel_attention_mask, hts_attn_mask)[0]
        
        #hts = self.htr_interactor_norm(hts + hts_1)
        #rels = self.rht_interactor_norm(rels + rels_1)
        

        # hts = hts.view(-1, hts.size(-1))[hts_attn_mask.view(-1)==1]
        # rels = rels.view(-1, rels.size(-1))[rel_attention_mask.view(-1)==1]

        hts_logits = contract("bld,brd->blr", hts, rels).contiguous()
        hts_logits = hts_logits.view(-1, hts_logits.size(-1))[hts_attn_mask.view(-1)==1]
